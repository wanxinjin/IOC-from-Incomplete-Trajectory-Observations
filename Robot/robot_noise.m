...
for k=1:length(stime)
    error(end+1)=SolveError(results(:,k),weights);
end
gamma
...
figure(1)
subplot(2,1,1)
plot(stime,oblength,'LineStyle','-','LineWidth',3)
ylabel('$l_{\min}(t)$','Interpreter','latex')
xlim([0,max(stime)])
grid on
subplot(2,1,2)
plot(stime,results(1,:),'LineStyle','-','LineWidth',3)
hold on
plot(stime,results(2,:),'LineStyle','--','LineWidth',3)
plot(stime,results(3,:),'LineStyle','-.','LineWidth',3)
legend('$\hat\omega_1$','$\hat\omega_2$', '$\hat\omega_2$','Interpreter','latex')
ylim([0 0.9])
xlim([0,max(stime)])
ylabel('$\hat\omega$', 'Interpreter','latex')
xlabel('Different observation starting time t','Interpreter','latex')
box on
grid on
